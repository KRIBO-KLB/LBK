...
def banner():
	...
	sol().print(cik2)
	ban=''' ➣AUTHOR       : MR.KRIBO     
   ➣GITHUB         : github.com/KRIBO-KLB
   ➣FACEBOOK   : Pemuja Kain Kafa'an
   ➣WHATSAPP  : +601169562396
   ➣ VERSION      : 2.0 •
...
def file():
	...
	print(x+'['+h+'R'+x+'] READING THE FILE, WAIT A MINUTE •••')
	time.sleep(2)
...
def dump_grup():
	...
	sol().print(au2)
	idgrup = input("[R] INPUT ID/USERNAME GRUP : ")
...
def pulkanid(linkmem):	
	...
	for mem in memberr:
		...
		sys.stdout.write('\r [R] COLLECTING  %s ID'%(len(id))); sys.stdout.flush()
